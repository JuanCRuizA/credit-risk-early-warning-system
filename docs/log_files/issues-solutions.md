...
### [ISSUE-020] case_studies.json Not Tracked in Git — Streamlit Cloud Warning Banner
**Date:** 2026-05-05
**Status:** Resolved
**Severity:** Medium
**Problem:** After committing NB04 changes, Tab 3 Case Study Explorer showed "Case studies file not found at /mount/src/credit-risk-early-warning-system/reports/case_studies.json" on Streamlit Cloud. The file existed locally but the dashboard was non-functional.
**Root Cause:** `reports/case_studies.json` was generated by running NB04 cell 32 (`case_studies_export`) locally but was never staged with `git add`. Git only commits explicitly staged files — untracked files are invisible to `git commit`. Streamlit Cloud reads exclusively from the git repository, so it had no access to the file.
**Solution:** `git add reports/case_studies.json`, then commit and push. Verified by searching GitHub for `.json` files — `case_studies.json` was absent from the repository.
**Prevention:** After generating any new output file from a notebook, verify it appears as staged in `git status` before committing. Run `git status reports/` to distinguish tracked from untracked files. Any file that app.py reads at runtime must be committed to git.
